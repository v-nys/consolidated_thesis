/*  $Id: chr_swi_bootstrap.pl,v 1.9 2006/04/22 17:56:17 toms Exp $
...
format_date(Out) :-
 	datime(DTime),
       DTime=datime(Year1,Month1,Day,Hour,Min,Sec),
       Year is 1900+Year1,
       Month is Month1+1,
...
%% B-Prolog begin
chr_local_only_read_term(A,B,C) :- read_term(A,B,[]).
chr_absolute_file_name(A,B,C) :- %nl,write(absolute_file_name(A,B,C)), nl, 
        A=C.  % works?
%% B-Prolog end
