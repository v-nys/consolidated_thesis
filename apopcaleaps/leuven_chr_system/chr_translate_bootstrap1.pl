...
attach_increment([O|P], v(G,A,B,C,D,E,F)) :-
        ...
        (   get_attr(O, chr_translate_bootstrap1, D1) ->
            D1=v(N,H,I,J,K,L,M),
            sort(H, A1),
            'chr merge_attributes'(A, A1, B1),
            sort(I, Y),
            'chr merge_attributes'(B, Y, Z),
            sort(J, W),
            'chr merge_attributes'(C, W, X),
            sort(K, U),
            'chr merge_attributes'(D, U, V),
            sort(L, S),
            'chr merge_attributes'(E, S, T),
            sort(M, Q),
            'chr merge_attributes'(F, Q, R),
            C1 is G\/N,
            put_attr(O, chr_translate_bootstrap1, v(C1,B1,Z,X,V,T,R))
...
chr_translate_bootstrap1:attr_unify_hook(v(G,A,B,C,D,E,F),H1) :-
        ...
        (   var(H1) ->
            (   get_attr(H1, chr_translate_bootstrap1, I1) ->
                ...
                G1 is G\/N,
                put_attr(H1, chr_translate_bootstrap1, v(G1,F1,D1,B1,Z,X,V)),
                'chr run_suspensions_loop'([F1,D1,B1,Z,X,V])
            ;   put_attr(H1, chr_translate_bootstrap1, v(G,T,S,R,Q,P,O)),
                'chr run_suspensions_loop'([T,S,R,Q,P,O])
            )
        ;   (   compound(H1) ->
                term_variables(H1, J1),
                attach_increment(J1, v(G,T,S,R,Q,P,O))
            ;   true
            ),
            'chr run_suspensions_loop'([T,S,R,Q,P,O])
        ).
...
'constraint_count/1__0'(A, B) :-
        'chr default_store'(E),
        get_attr(E, chr_translate_bootstrap1, D),
        D=v(I,_,H,_,_,_,_),
        I/\2=:=2,
        'chr sbag_member'(C, H),
...
'get_max_constraint_index/1__0'(A, _) :-
        'chr default_store'(D),
        get_attr(D, chr_translate_bootstrap1, C),
        C=v(H,_,_,_,G,_,_),
        H/\8=:=8,
        'chr sbag_member'(B, G),
...
'chr_clear/0__0'(A) :-
        'chr default_store'(C),
        get_attr(C, chr_translate_bootstrap1, B),
        B=v(E,D,_,_,_,_,_),
        E/\1=:=1, !,
        (   var(A) ->
