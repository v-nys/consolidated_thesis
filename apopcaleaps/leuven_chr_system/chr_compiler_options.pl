/*  $Id: chr_compiler_options.pl,v 1.14 2006/04/11 14:21:17 toms Exp $
...
option_definition(all_ground,off,Flags) :-
	Flags = [ all_ground - off ].
option_definition(all_ground,on,Flags) :-
	Flags = [ all_ground - on ].
...
option_definition(reorder_heads,on,Flags) :-
	Flags = [reorder_heads - on].
option_definition(reorder_heads,off,Flags) :-
	Flags = [reorder_heads - off].
...
chr_pp_flag_definition(all_ground,[off,on]).
