...
'$allocate_constraint_leq/2'(A, suspension(B,C,D,G,A,_,E,F), E, F) :-
...
'leq/2__0'(A, B, C) :-
        ...
        (   var(C) ->
            true
        ;   '$remove_constraint_internal_leq/2'(C, D, E),
            (   E==yes ->
                '$delete_from_store_leq/2'(C),
                'detach_leq/2'(D, C)
            ;   true
            )
        ).
'leq/2__0'(A, B, C) :-
        (   'chr newvia_2'(B, A, I) ->
            get_attr(I, leq, G)
        ;   nb_getval('$chr_store_global_list_leq:leq/2', G)
        ),
        'chr sbag_member'(D, G),
        D=suspension(_,H,_,_,_,_,E,F),
        H=mutable(active),
        E==B,
        F==A, !,
        ...
        (   var(C) ->
            true
        ;   '$remove_constraint_internal_leq/2'(C, L, M),
            (   M==yes ->
                '$delete_from_store_leq/2'(C),
                'detach_leq/2'(L, C)
            ;   true
...
'leq/2__0'(A, B, C) :-
        ...
        (   var(C) ->
            true
        ;   '$remove_constraint_internal_leq/2'(C, J, K),
            (   K==yes ->
                '$delete_from_store_leq/2'(C),
                'detach_leq/2'(J, C)
            ;   true
...
'leq/2__0'(A, B, C) :-
        (   'chr newvia_1'(B, E) ->
            get_attr(E, leq, D)
        ;   nb_getval('$chr_store_global_list_leq:leq/2', D)
        ), !,
        (   var(C) ->
            '$allocate_constraint_leq/2'(leq:'leq/2__0'(A,B,C), C, A, B)
        ;   true
        ),
        'leq/2__0__0'(D, A, B, C).
'leq/2__0__0'([], B, C, A) :-
        'leq/2__1'(B, C, A).
'leq/2__0__0'([G|H], B, C, A) :-
        (   G=suspension(_,F,_,_,_,_,D,E),
            ...
            '$extend_history'(A, N),
            '$activate_constraint_leq/2'(K, J, A, I),
            (   K==yes ->
                '$insert_in_store_leq/2'(A),
                'attach_leq/2'(J, A)
            ;   true
